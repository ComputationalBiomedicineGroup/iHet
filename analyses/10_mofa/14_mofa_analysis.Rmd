...
tracerx_factors <- lapply(c("JiaSharma", "TCGA-NSCLC"), function(weights_source) {
  mofa_data_tracerx %>%
...
# same for tracerx (once as derived by mofa, once as calculated using TCGA-NSCLC signatures)

selected_datasets <- c("TRACERx", "TRACERx (TCGA-NSCLC)")
factors_all %>%
  inner_join(immresp_all) %>%
  filter(dataset_label %in% selected_datasets, factor == "factor 1") %>%
  mutate(dataset_label = factor(dataset_label, levels = selected_datasets)) %>%
...
lapply(c("TCGA-NSCLC", "LUAD", "LUSC", "Jia", "Sharma", "JiaSharma"), function(dataset_id) {
  cd8_all %>%
    inner_join(tmb_all) %>%
...
all_weights %>%
  filter(
    ...
    factor == "factor 1",
    dataset %in% c("Jia", "Sharma", "JiaSharma", "LUAD", "LUSC", "TCGA-NSCLC", "GTEx")
  ) %>%
