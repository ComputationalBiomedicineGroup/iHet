...
features_tracerx <- readRDS(file.path(nxfvars$features_dir, "TRACERx.rds"))
```
...
tmb_all <- lapply(c("TCGA_TMB.rds", "Sharma_TMB.rds", "Jia_TMB.rds"), function(tmb_file) {
  ...
  tibble(patient = names(tmp_tmb), TMB = tmp_tmb) %>%
    mutate(patient = str_replace_all(patient, "-", "."))
}) %>% bind_rows()

correlation_data %>%
  inner_join(tmb_all) %>%
  ggplot(aes(x = factor_value, y = TMB)) +
  geom_point(size = 0.8, color = "#333333") +
  theme_bw() +
  stat_cor() +
  geom_smooth(method = "lm") +
...
feature_data <- list(TRACERx = features_tracerx, JiaSharma = features_jiasharma)
