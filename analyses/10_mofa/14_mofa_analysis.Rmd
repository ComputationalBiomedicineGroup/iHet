...
features_jiasharma = readRDS("../../data/results/10_mofa/12_prepare_mofa_data/datasets/JiaSharma.rds")
```
...
features_jiasharma %>% 
  ggplot(aes(x=value, y=patient, fill=group)) + 
    geom_boxplot() + facet_wrap(~feature, scale="free_x") + 
...
get_df = function(df, group) {
    bf= anovaBF(value~patient, data = df) %>% as.vector()
...
bfs %>% mutate(log_bf = log2(bayes_factor)) %>% 
