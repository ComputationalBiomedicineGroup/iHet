...
dir.create(file.path(nxfvars$out_dir, "plots"), showWarnings = FALSE)
...
feature_data <- readRDS(file.path(nxfvars$features_dir, "data_all_tidy.rds"))

tmb_all <- lapply(c("TCGA_TMB.rds", "Sharma_TMB.rds", "Jia_TMB.rds"), function(tmb_file) {
  tmp_tmb <- readRDS(file.path(nxfvars$tmb_dir, tmb_file))
...
models <- sapply(DATASETS, function(dataset_id) {
  load_model(file.path(nxfvars$data_dir, sprintf("mofa_%s.hdf5", dataset_id)))
}, simplify = FALSE, USE.NAMES = TRUE)

bootstrap_models <- sapply(DATASETS, function(dataset_id) {
  lapply(Sys.glob(file.path(nxfvars$data_dir, sprintf("mofa_boot_%s_*.hdf5", dataset_id))), function(x) {
    load_model(x)
  })
}, simplify = FALSE, USE.NAMES = TRUE)
```
...
saveRDS(median_weights, file = file.path(nxfvars$out_dir, "median_weights.rds"))
```
...
all <- buildHeatmap(get_weight_df(models$Jia), "Jia") +
  ...
  buildHeatmap(get_weight_df(models$JiaSharma), "JS") +
...
lapply(names(factor_list), function(factors_name) {
  ...
  write_tsv(
    ...
    file.path(nxfvars$out_dir, sprintf("%s_factor_correlations.tsv", factors_name))
  )
  write_tsv(
    cor_pmat %>% as_tibble(rownames = "factor"),
    file.path(nxfvars$out_dir, sprintf("%s_factor_correlation_pvalues.tsv", factors_name))
  )
})
```

### Correlation with immune response
```{r, fig.width=16, fig.height=14, echo=FALSE, message=FALSE, warning=FALSE}
...
correlation_data <- lapply(names(models), function(dataset_id) {
...
correlation_data %>%
  ...
  facet_grid(rows = vars(dataset), cols = vars(factor), scales = "free_x") +
  ylim(c(-2, 3))
```
...
lapply(correlation_data$dataset %>% unique(), function(dataset_id) {
  correlation_data %>%
    inner_join(tmb_all) %>%
    ...
    theme_cowplot() +
    ylim(c(0, 2))
  ggsave(file.path(nxfvars$out_dir, "plots", sprintf("correlation_f1_tmb_%s.png", dataset_id)), width = 4, height = 4)
})
```
...
tmp_scatter_data %>%
  ...
  geom_label_repel(data = tmp_scatter_data %>% filter(abs(JiaSharma) > 0.5 | abs(NSCLC) > 0.5), aes(label = feature)) +
  theme_bw() +
