...
mofa_data_tracerx <- readRDS(file.path(nxfvars$features_dir, "mofa_TRACERx.rds"))
...
lapply(factors_all$dataset %>% unique(), function(dataset_id) {
  factors_all %>%
    inner_join(immresp_all) %>%
    ...
    ylim(c(-2, 3)) +
    ggtitle(dataset_id)
  ggsave(file.path(nxfvars$out_dir, "plots", sprintf("correlation_f1_immune_response_%s.png", dataset_id)), width = 4, height = 4)
})
```
...
tmp_scatter_data %>%
  ...
  geom_label_repel(size = 3.5, data = tmp_scatter_data %>% filter(abs(JiaSharma) > 0.5 | abs(NSCLC) > 0.5), aes(label = feature)) +
