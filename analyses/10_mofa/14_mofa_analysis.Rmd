...
SELECTED_DATASETS <- c("Jia", "Sharma", "JiaSharma", "LUAD", "LUSC", "NSCLC", "GTEx", "TRACERx")
...
tmb_tcga <- read_tsv(file.path(params$tmb_dir, "TCGA_mutation-load_updated.txt")) %>%
  ...
  pivot_longer(-sample, names_to = "metric", values_to = "tmb_value")
tmb_all <- bind_rows(tmb_jia_sharma, tmb_tcga)
```
...
# remove TRACERx for downstream analysis
models$TRACERx <- NULL
median_weights$TRACERx <- NULL
median_factors$TRACERx <- NULL
```
...
```{r correlation_features, echo = FALSE,  fig.width=15, fig.height=16, fig.align="center", out.width="80%", fig.cap = "Heatmap depicting the correlation of all factors of all models based on feature data. Only significant relations (FDR < 0.01) are shown."}
...
ggsave(file.path(params$artifact_dir, "plots", "correlation_factors_immune_response.pdf"), width = 18, height = 22)
```


### Correlation: factors vs. tmb
```{r, fig.height=12, fig.width=12}
lapply(c("NSCLC", "LUAD", "LUSC", "Jia", "Sharma", "JiaSharma"), function(dataset_id) {
  message(dataset_id)
  factors_all %>%
    filter(dataset_label == dataset_id) %>%
    inner_join(tmb_all) %>%
    ...
    stat_cor() +
    geom_smooth(method = "lm") +
    facet_wrap(metric ~ factor, scales = "free_x", ncol = 7)
  ggsave(file.path(params$artifact_dir, "plots", sprintf("correlation_factors_tmb_%s.pdf", dataset_id)), width = 18, height = 9, title = sprintf("Correlation Factors vs. TMB %s", dataset_id))
})
```


### Correlation: factors vs. CD8 T 
...
factors_all %>%
  ...
  stat_cor() +
  geom_smooth(method = "lm") +
  facet_wrap(dataset ~ factor, scales = "free_x", ncol = 7)

ggsave(file.path(params$artifact_dir, "plots", "correlation_factors_CD8.pdf"), width = 18, height = 22)
```

### Correlation: CD8 vs. TMB
```{r, fig.width=16, fig.height=7}
lapply(c("NSCLC", "LUAD", "LUSC", "Jia", "Sharma", "JiaSharma"), function(dataset_id) {
  cd8_all %>%
    inner_join(tmb_all) %>%
    filter(dataset == dataset_id) %>%
    ggplot(aes(x = tmb_value, y = `CD8 T`)) +
    geom_point(size = 0.8, color = "#333333") +
    theme_bw() +
    stat_cor() +
    geom_smooth(method = "lm") +
  ...
  ggsave(file.path(params$artifact_dir, "plots", sprintf("correlation_CD8_TMB_%s.pdf", dataset_id)), width = 8, height = 4, title = sprintf("Correlation CD8 TMB %s", dataset_id))
})
```

### Correlation: Immune response vs. TMB
```{r, fig.width=16, fig.height=7}
lapply(c("NSCLC", "LUAD", "LUSC", "Jia", "Sharma", "JiaSharma"), function(dataset_id) {
  message(dataset_id)
  immresp_all %>%
    inner_join(tmb_all) %>%
    filter(dataset == dataset_id) %>%
    ggplot(aes(x = tmb_value, y = `immune response`)) +
    geom_point(size = 0.8, color = "#333333") +
    theme_bw() +
    stat_cor() +
    geom_smooth(method = "lm") +
  ...
  ggsave(file.path(params$artifact_dir, "plots", sprintf("correlation_immune_response_TMB_%s.pdf", dataset_id)), width = 8, height = 4, title = sprintf("Correlation Immune-response TMB %s", dataset_id))
})
```


### Correlation: CD8 T vs immune response
...
ggsave(file.path(params$artifact_dir, "plots", "correlation_CD8_immune_response.pdf"), width = 18, height = 13)
```
...
tmp_scatter_data %>%
  ...
  geom_label_repel(size = 3.5, data = tmp_scatter_data %>% filter(JiaSharma > 0.5 | NSCLC > 0.5 | NSCLC < -0.3 | JiaSharma < -0.3), aes(label = feature)) +
...
ggsave(file.path(params$artifact_dir, "plots", "weight_scatter.pdf"), width = 18, height = 14)
```
