...
factors_all %>%
  ...
  filter(dataset %in% c("Jia", "Sharma", "JiaSharma", "LUAD", "LUSC", "NSCLC", "GTEx")) %>%
...
selected_features <- tmp_scatter_data %>%
  filter(JiaSharma > 0.5 | NSCLC > 0.5 | NSCLC < -0.3 | JiaSharma < -0.3) %>%
  pull(feature)
...
all_weights %>%
  filter(
    ...
    factor == "factor 1",
    dataset %in% c("Jia", "Sharma", "JiaSharma", "LUAD", "LUSC", "NSCLC", "GTEx")
  ) %>%
