...
selected_datasets <- c("Jia", "Sharma", "JiaSharma", "LUAD", "LUSC", "NSCLC", "GTEx")
...
dataset_list <- c("Jia", "Sharma", "JiaSharma", "LUAD", "LUSC", "NSCLC", "GTEx")
...
```{r, fig.width=3, fig.height=5.5, echo=FALSE, message=FALSE, warning=FALSE}
# same for tracerx (once as derived by mofa, once as calculated using NSCLC signatures)

selected_datasets <- c("TRACERx", "TRACERx (NSCLC)")
factors_all %>%
  inner_join(immresp_all) %>%
  filter(dataset_label %in% selected_datasets, factor == "factor 1") %>%
  mutate(dataset_label = factor(dataset_label, levels = selected_datasets)) %>%
...
factors_all %>%
  inner_join(tmb_all) %>%
  filter(dataset %in% selected_datasets, factor == "factor 1", str_to_lower(metric) == "tmb") %>%
  mutate(dataset_label = factor(dataset_label, levels = selected_datasets)) %>%
...
tmp_scatter_data %>%
  ...
  geom_label_repel(size = 3.5, force = 3, max.time = 10, max.overlaps = 300, data = tmp_scatter_data %>% filter(JiaSharma > 0.5 | NSCLC > 0.5 | NSCLC < -0.3 | JiaSharma < -0.3), aes(label = feature)) +
...
inter_sample <- feature_data %>%
  filter(dataset == "JiaSharma") %>%
  group_by(view, feature, patient) %>%
...
variability_df <- inter_sample %>%
  inner_join(intra_sample, by = c("view", "feature")) %>%
...
```{r}
variability_df %>%
  ggplot(aes(x = mad_intra, y = mad_inter, color = view)) +
  geom_point() +
  theme_cowplot() +
  background_grid() +
  scale_color_brewer(palette = "Dark2") +
...
```{r}
feature_stability_df %>%
  inner_join(variability_df, by = c("feature", "view")) %>%
