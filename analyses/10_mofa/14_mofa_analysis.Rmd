...
._ = lapply(list(NSCLC=filter_nsclc, other=filter_other), function(f) {
  sample_tab = feature_data %>% f() %>% select(sample, patient, dataset) %>% distinct()
  sample_count = sample_tab %>% group_by(dataset) %>% count()
  print(sample_count)
  print(sum(sample_count$n))

  patient_count = sample_tab %>% select(-sample) %>% distinct() %>% group_by(dataset) %>% count()
  print(patient_count)
