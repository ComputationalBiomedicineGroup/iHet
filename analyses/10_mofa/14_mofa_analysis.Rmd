...
if (INCLUDE_TRACERX) {
  SELECTED_DATASETS <- c(SELECTED_DATASETS, "TRACERx")
}
...
if (INCLUDE_TRACERX) {
  ...
  tmb_all <- bind_rows(tmb_all, tmb_tracerx)
}
```
...
## Summary stats
```{r}
filter_nsclc <- function(x) filter(x, dataset %in% c("Jia", "Sharma", "TRACERx", "LUAD", "LUSC"))
filter_other <- function(x) filter(x, !dataset %in% c("Jia", "Sharma", "TRACERx", "LUAD", "LUSC", "JiaSharma", "GTEx", "TCGA-NSCLC"))
._ <- lapply(list(NSCLC = filter_nsclc, other = filter_other), function(f) {
  sample_tab <- feature_data %>%
    f() %>%
    select(sample, patient, dataset) %>%
    distinct()
  sample_count <- sample_tab %>%
    group_by(dataset) %>%
    count()
  print(sample_count)
  print(sum(sample_count$n))

  patient_count <- sample_tab %>%
    select(-sample) %>%
    distinct() %>%
    group_by(dataset) %>%
    count()
  print(patient_count)
  print(sum(patient_count$n))
})
```
...
svg(file.path(params$artifact_dir, "plots", "heatmap_bootstrapped.svg"), width = 15, height = 98)
...
selected_features <- tmp_scatter_data %>%
  ...
  filter(JiaSharma > 0.6 | `TCGA-NSCLC` > 0.6 | `TCGA-NSCLC` < -0.6 | JiaSharma < -0.6) %>%
  pull(feature)
...
sample_to_patient_mapping <- feature_data %>%
  select(sample, patient) %>%
  distinct()
tmp_datasets <- "JiaSharma"
if (INCLUDE_TRACERX) tmp_datasets <- c(tmp_datasets, "TRACERx")
lapply(tmp_datasets, function(dataset_id) {
  median_factors[[dataset_id]] %>%
    inner_join(sample_to_patient_mapping, by = "sample") %>%
    select(patient, `factor 1`) %>%
    ggplot(aes(x = `factor 1`, y = `patient`)) +
...
variability_df <- inter_sample %>%
  ...
  mutate(ratio = log2(mad_inter / mad_intra))
```
...
tmp_datasets <- c("Jia", "Sharma", "JiaSharma")
if (INCLUDE_TRACERX) tmp_datasets <- c(tmp_datasets, "TRACERx")
lapply(tmp_datasets, function(dataset) {
  variability_df %>%
...
Correlation between rank-based and inter-intra-ratio based measures
