...
params:
    ...
    dataCorHIF: "../../../../repo_pred/iHet_predictions/data/correlation_features_fibroblasts_tumor.RDS"
...
## Correlation TCGA features with fibroblast density (NSCLC)
### Filter for FDR <= 0.05 & corr > 0)
feat_cor_hif <- feat_cor_hifs[["NSCLC"]] %>% 
  group_by(feature, cancertype, hif) %>%
  filter(FDR <= 0.05 && cor > 0 && cancertype == "NSCLC")
...
colnames(iHet_df) <- c("patient", "score", "hif_tissue", "method", "run", "model", "hif_model", "dataset")

# Unknown histology, average LUAD and LUSC iHet scores is used
## normalize to -1,1
iHet_df_norm <- iHet_df %>% 
  dplyr::group_by(method, run, hif_model) %>% 
  dplyr::mutate(scaled_score = linear_func(score, "11")) %>%
  dplyr::group_by(patient, method, run, hif_tissue, dataset) %>% 
...
PRcurve <- assess_predictive_performance(iHet=iHet_df_norm,
                                         response = response,
                                         use_bootstrap_weights=TRUE,
                                         method = c("iHet", "iHet_inverted"),
                                         use_fibroblasts_hifs_tissue = c("tumor"),
...
df_tcga_weights <- readRDS(file = "../../../../repo_pred/iHet_predictions/data/all_tcga_iHet_weights_bootstrap_noDCandMono_scaled.RDS")
...
barplot_double_PeCorr <- ggplot2::ggplot(cor_df, aes(x=CancerType_n, y=PeCorr, fill = analysis)) +
  ggplot2::geom_bar(stat = "identity", color = "black", position = position_dodge2()) +
  ggplot2::scale_fill_manual(values = c("#E0C469", "#9B243A")) +
  ggplot2::theme_classic() +
  ggplot2::ylim(c(0,1)) +
  ggplot2::theme(axis.title.y = element_blank(), strip.background = element_blank(), 
  ...
  ggplot2::labs(y = "Pearson correlation")
...
gg1 <- frac_pat_response %>% filter(is.na(MFP) == FALSE) %>%
...
gg2 <- frac_pat_response %>% filter(is.na(MFP) == FALSE) %>%
  ggplot2::ggplot(aes(x = category, y = counts, fill = MFP, label = counts)) +
  ggplot2::geom_bar(position = "stack", stat="identity") +
  scale_fill_manual(values = c("#64a860", "#9970c1","#b98d3e","#cc545e")) +
  geom_text(size = 3, position = position_stack(vjust = 0.5), color = "white") +
...
auc_df <- do.call(rbind, lapply(names(datasets_bulk), function(dataset){
  #cat("Dataset:", dataset, "\n")
      ...
      if (dataset %in% c("mariathasan", "gideauslanderpd1", "kim", "jung.nsclc")){
        # iHet scores
        iHet_df <- readRDS(paste0("../../../../repo_pred/iHet_predictions/analysis/iHet_ms/", dataset,"_iHet_scores_with_hifs_model_",
                                  tolower(hif_type),"_using_scaled_weights.RDS"))
      ...
      auc_df <- assess_predictive_performance(iHet=iHet_df,
                                              ...
                                              assess_cor_threshold = FALSE, 
                                              plot_roc=FALSE,
                                              metric="roc") #roc, precision_recall
...
stat.test <- auc_df %>%
  group_by(dataset) %>%
  rstatix::wilcox_test(auc ~ method, ref.group = "iHet", paired = TRUE, alternative = "less") %>%
  add_significance() %>% 
  add_xy_position(x = "dataset")

roc_perf <- ggplot(auc_df, aes(x=dataset, y=auc, fill = method)) +
  ...
  #cat("Dataset:", dataset, "\n")
     # cat("Cancer type hifs:", hif_type, "\n")
      ...
      auc_df <- assess_predictive_performance(iHet=iHet_df,
                                              ...
                                              assess_cor_threshold = FALSE, 
                                              plot_roc=FALSE,
                                              metric="precision_recall") #roc, precision_recall
...
pr_perf <- ggplot(auc_df, aes(x=dataset, y=auc, fill = method)) +
  geom_boxplot() + #geom_sina(alpha=0.5) +
  scale_y_continuous(limits=c(0, 1)) +
  theme_bw() +
  scale_x_discrete(labels= new_labels) +
...
gridExtra::grid.arrange(roc_perf, pr_perf, nrow=1)
...
plots <- lapply(names(datasets_bulk), function(dataset){
  plots <- lapply(c("tumor"), function(tissue){
    ...
    # iHet scores
    iHet_df <- readRDS(paste0("../../../../repo_pred/iHet_predictions/analysis/iHet_ms/", 
                              dataset,"_iHet_scores_with_hifs_model_", tolower(hif_type),"_using_scaled_weights.RDS"))
...
## Example on how to compute iHet scores for a given dataset
