...
for (model_sel in model){
  ...
  counts <- solve_annotation_issues(gene_expr = tcga_features$count[[model_sel]])
  tpm <- solve_annotation_issues(gene_expr = tcga_features$tpm[[model_sel]])
...
luad_patients <- substr(gsub(".", "-", colnames(tcga_features$tpm$LUAD), fixed = TRUE), 1, 12)
lusc_patients <- substr(gsub(".", "-", colnames(tcga_features$tpm$LUSC), fixed = TRUE), 1, 12)
...
colnames(tcga_iHet_mat)[4] <- "iHet_rev"
...
OS_poplaroak <- ggsurvplot(fit_poplaroak, 
                           ...
                           risk.table.col = "strata", h) # Risk table color by groups
## iHet_rev
