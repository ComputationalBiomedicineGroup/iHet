...
```{r}
# TODO: tmp check
dorothea_tfs <- readr::read_rds("/home/sturm/projects/2021/nsclc_heterogeneity/analyses/10_mofa/dorothea_tfs_old.rds")
gtex_healthy_samples <- readr::read_tsv("/home/sturm/projects/2021/nsclc_heterogeneity/data/01_processed/bulk_rna_seq/GTEx_healthy_samples.tsv")
gtex_accession_numbers <- readr::read_csv("/home/sturm/projects/2021/nsclc_heterogeneity/data/01_processed/bulk_rna_seq/GTEx_accession_numbers.csv")
gtex_accession_numbers <- gtex_accession_numbers %>%
  select(`Library ID` = Experiment, gtex_id = biospecimen_repository_sample_id) %>%
  distinct()
gtex_healthy_sample_ids <- gtex_accession_numbers %>%
  semi_join(gtex_healthy_samples) %>%
  pull(gtex_id)
```
...
processFeatures <- function(data, dataset_id) {
  ...
  pathways.df <- do_pivot(data$pathway[[dataset_id]], "Pathway scores")
  # TODO filter is tmp_check
  tfs.df <- do_pivot(data$tf[[dataset_id]], "Transcription factors") # |> filter(feature %in% dorothea_tfs)
...
# TODO
# subset gtex to 'truly healthy' samples
selected_gtex_idx <- intersect(colnames(gtex_data[["tpm"]]$Lung), gtex_healthy_sample_ids)
gtex_data[["tpm"]][["Lung"]] <- gtex_data[["tpm"]][["Lung"]][, selected_gtex_idx]
gtex_data[["count"]][["Lung"]] <- gtex_data[["count"]][["Lung"]][, selected_gtex_idx]
gtex_data[["pathway"]][["Lung"]] <- gtex_data[["pathway"]][["Lung"]][selected_gtex_idx, ]
gtex_data[["tf"]][["Lung"]] <- gtex_data[["tf"]][["Lung"]][selected_gtex_idx, ]
gtex_data[["cellfrac"]][["Lung"]] <- gtex_data[["cellfrac"]][["Lung"]][selected_gtex_idx, ]
gtex_data[["immresp"]][["Lung"]] <- gtex_data[["immresp"]][["Lung"]][selected_gtex_idx, ]
...
# Subset TFs to intersection across all datasets
# In some datasets, a few TFs (like 4)
tfs_intersection <- Reduce(intersect, lapply(data_all$tf, colnames))
data_all$tf <- sapply(data_all$tf, function(tmp_tf) {
  tmp_tf[, tfs_intersection]
}, USE.NAMES = TRUE, simplify = FALSE)
```
...
lapply(unique(data_all_tidy$view), function(tmp_view) {
  print(tmp_view)
  tmp_data <- data_all_tidy %>%
    dplyr::filter(view == !!tmp_view) %>%
    tidyr::pivot_wider(id_cols = c("sample", "dataset"), names_from = "feature", values_from = "value") %>%
    dplyr::select(-sample, -dataset)

  p <- GGally::ggpairs(tmp_data,
    columns = head(colnames(tmp_data), 15),
    lower = list(
      continuous = GGally::wrap("smooth", size = 0.05, alpha = 0.05)
    )
  )

  print(p)
  #   ggplot2::ggsave(file.path(params$artifact_dir, sprintf("feature_distributions_%s.pdf", tmp_view)), width = 15, height = 15)
})
```
