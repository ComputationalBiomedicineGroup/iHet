...
processRNAseq <- function(counts, regression = 1, group = NULL) {
  ...
  if (str_contains(colnames(PatientData), "TCGA") || str_contains(colnames(PatientData), "pbmc")) {
    ...
    colnames(PatientData) <- sapply(colnames(PatientData), function(x) {
      if (str_length(x) == 5) {
        substr(x, 1, 2)
      } else {
        substr(x, 1, 4)
    ...
    # Add group
    RNAseq.norm.df$group <- sapply((RNAseq.norm.df$sample), function(x) {
      if (str_length(x) == 5) {
        "Sharma"
      } else {
        "Jia"
    ...
    if (regression == 1) {
      ...
      dataLM$patient <- as.factor(sapply(dataLM$sample, function(x) {
        if (str_length(x) == 5) {
          substr(x, 1, 2)
        } else {
          substr(x, 1, 4)
...
processFeatures <- function(data, group = NULL) {
  ...
  Patients <- sapply(colnames(data$counts), function(x) {
    if (str_length(x) == 5) {
      substr(x, 1, 2)
    } else {
      substr(x, 1, 4)
...
generateHeatmap <- function(data, title, row_names = F) {
  ...
  hm <- Heatmap(data,
    column_title = title, col = col_fun, top_annotation = top_ann, show_row_dend = F, column_title_gp = gpar(fontsize = 18), row_names_gp = gpar(fontsize = 17), column_names_gp = gpar(fontsize = 17),
...
runGSEA <- function(model, feature_set, factors, p = 0.1) {
  ...
  enrichment.merged <- apply(enrichment.merged, c(1, 2), function(x) {
    if (x < p) {
      x <- x
    } else {
      x <- NA
...
plotGSEA <- function(list_of_GSEA_results, type) {
  getTopGenes <- function(single_model) {
    return(unlist(lapply(seq(6), function(x) {
      rownames(single_model[x])[which.maxn(single_model[, x], 10)]
    })))
  }


  if (type == "GO") {
    genes <- table(unlist(lapply(list_of_GSEA_results, function(x) {
      getTopGenes(x)
    })))
    genes <- genes[genes > 1]
    ...
    genes <- unique(unlist(lapply(list_of_GSEA_results, function(x) {
      rownames(x)
    })))
  }
  ...
  getGSEAHeatmap <- function(model, title) {
    ...
    col_fun <- circlize::colorRamp2(seq(0, max.lim, by = (max.lim) / 5), c(
...
plotResponseCorr <- function(model, type, corr) {
  ...
  # Generate figure
  fig <- (ggscatter(table,
    y = "proxy", x = "value", add = "reg.line", conf.int = TRUE, facet.by = "Factor",
    cor.coef = TRUE, cor.method = "pearson", formula = "x ~ y",
    xlab = paste0("Factor values"), ylab = "Response score", size = 1, height = 1
...
responseHeatmap <- function(type) {
  ...
  setsize <- 0.6
  col_fun <- circlize::colorRamp2(seq(0, 1, by = 1 / 10), c(
    "#67001F", "#B2182B", "#D6604D", "#F4A582",
    "#FDDBC7", "#FFFFFF", "#D1E5F0", "#92C5DE",
    "#4393C3", "#2166AC", "#053061"
  ))
...
get_median_bootstrap_data <- function(data, variance_cutoff = 0) {
  ...
  data <- as.data.frame(do.call(cbind, lapply(seq(1), function(x) {
    getMedianperFactor(data, x)
  })))
...
Bootstrap_MOFA <- function(type, N) {
  ...
  mofa_data <- as.data.frame(do.call(rbind, lapply(seq(length(samples)), function(x) {
    getSampleFromData(x)
  })))
...
plotWeightsFactors <- function(model1, model2, xlabel, ylabel) {
  ...
  legend("topright", legend = c(xlabel, ylabel), col = c(
    rgb(1, 0, 0, 0.5),
    rgb(0, 0, 1, 0.5)
  ), pt.cex = 2, pch = 15)
...
plotHorBarDiff <- function(y, x) {
  ...
  ggplot(diff, aes(x = key, y = value)) +
    geom_bar(stat = "identity") +
    coord_flip() +
    scale_y_continuous(name = "Difference in weight") +
...
plotRawFeatures <- function(dataset, type) {
  ...
  ggplot(data = df, aes(x = X, y = Y)) +
    geom_point() +
    scale_y_log10(limits = c(1e-02, 1e3)) +
    scale_x_log10(limits = c(1e-02, 1e+01))



  ggplot(data = df, aes(x = X, y = V2)) +
    geom_point() +
    scale_y_log10(limits = c(1e-02, 1e3)) +
    ...
    theme_bw() +
    ggtitle(paste0("Within- vs between-patient variability ", type))
}
...
plotOverview <- function(jiasharma, nsclc) {
  ...
  plot <- ggplot(data, aes(JS, NSCLC), add = "reg.line", conf.int = TRUE) + # ggtitle("Median of factor 1 weights Jia-Sharma and NSCLC dataset") +
    ...
    geom_text_repel(aes(label = labels), size = 6, face = "bold")
...
plotVolcano <- function(jiasharma, nsclc, labels) {
  ...
  g <- ggplot(data = results, aes(x = effsize, y = -log10(pvalue)), environment = environment()) +
    ...
    geom_point(data = subset_results, aes(fill = Dataset, size = Specificity), color = "black", alpha = 0.8, shape = 21) +
    scale_fill_manual(values = c("purple", "blue", "red")) +
    scale_size_manual(values = c(5, 3)) +
    xlim(c(-xminmax, xminmax)) +
    ylim(c(0, ymax)) +
    xlab("larger in NSCLC                 effect size             larger in Jia-Sharma") +
    ylab("-log10 p-value") +
    ggtitle("") +
    theme_bw() +
    geom_hline(yintercept = -log10(0.05), linetype = "longdash", colour = "#9e9e9e") +
    geom_vline(xintercept = 0, linetype = "solid", colour = "#9e9e9e") +
...
plotBeeswarm <- function(labels, dataset) {
  ...
  # Separate patients
  patients <- sapply(rownames(data), function(x) {
    if (str_length(x) == 9) {
      substr(x, 1, 2)
    } else {
      substr(x, 1, 4)
  ...
  getBeeswarm <- function(feature) {
    return(ggplot(data, aes_string(x = "patient", y = feature, col = "patient")) +
      geom_beeswarm(size = 3, groupOnX = FALSE, priority = "density") +
